...
```sql
- Gold (long) - 골드 보유량
- LastLoginTime (DateTime) - 마지막 로그인 시간
```
...
### 단위 테스트
- `OfflineRewardServiceTests.cs` (17개 테스트 통과)
- 전투 시스템 테스트 (일부 작성됨)

### 통합 테스트
- 로컬 환경에서 Swagger를 통한 수동 테스트 완료
- 전투 시뮬레이션 정상 작동 확인
- 오프라인 보상 수령 시나리오 검증
...
### 마이그레이션
- Jenkins가 자동으로 RDS에 마이그레이션 적용
- `migration.sql` 스크립트 기반
...
### 2025-10-16: Feature 6 방향 전환
- **결정**: IHostedService 실시간 자동 전투 → 경과 시간 기반 보상
- **이유**: 서버 리소스 효율화, 확장성 개선
- **영향**: Feature 6은 스케줄러 서버 구현 시 일일 리셋으로 대체

### 2025-10-16: AI Consensus 작업 순서 결정
- **결정**: Week 2 완료 → Inventory 시작 (병렬 작업 없이)
- **모델**: Gemini 2.5 Pro, Grok-4 (GPT-5-Pro는 크레딧 부족)
- **합의**: Vertical Slice 원칙, 논리적 종속성 (보상 → 인벤토리)
...
- Week 2 PRD: `.taskmaster/docs/week2-prd.txt`
- CLAUDE.md: 프로젝트 컨텍스트
- Task Master: `.taskmaster/tasks/tasks.json`
- Tech Stack Memory: Serena `tech_stack`
- Development Roadmap: Serena `development_roadmap_checklist`
