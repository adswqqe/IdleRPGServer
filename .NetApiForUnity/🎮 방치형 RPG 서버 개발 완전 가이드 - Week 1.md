...
#### 환경 설정 체크리스트:
- [ ] **Rider IDE** 실행 및 프로젝트 로드 성공
- [ ] **Docker Desktop**에서 PostgreSQL 컨테이너 실행 중 
- [ ] **pgAdmin** 웹 접속 및 데이터베이스 연결 성공
- [ ] **GitHub** 저장소 연결 및 첫 커밋 완료
- [ ] **Swagger UI** 접속 가능 (`https://localhost:7260/swagger`)

#### 문제 해결 가이드:

**🔴 PostgreSQL 연결 실패 시:**
1. Docker Desktop에서 `idlerpg-postgres` 컨테이너 상태 확인
2. 포트 5432가 사용 중인지 확인: `netstat -an | findstr 5432`
3. 방화벽에서 5432 포트 허용 확인

**🔴 Rider에서 NuGet 패키지 설치 실패 시:**
1. `Tools` → `NuGet` → `Package Manager Console`
2. `Clear All NuGet Cache(s)` 실행
3. 솔루션 다시 빌드: `Build` → `Rebuild Solution`

**🔴 GitHub 연결 실패 시:**
1. `File` → `Settings` → `Version Control` → `GitHub`
2. 계정 재인증 또는 Personal Access Token 사용

---

## ⚡ 빠른 시작 가이드 (5분 설정)

### Unity 개발자를 위한 완전 자동화 설정 🎯

**프로젝트가 이미 GitHub에 준비되어 있다면:**

#### Step 1: 저장소 클론 (Rider GUI)
1. **Rider 시작 화면** → `Get from VCS`
2. **URL**: `https://github.com/yourusername/IdleRPGServer.git`
3. **Directory**: 원하는 로컬 경로
4. **Clone** 클릭

#### Step 2: Docker 서비스 실행 (한 번의 클릭)
1. **Rider 터미널** → `docker-compose up -d postgres redis pgadmin`
2. **또는** Docker Desktop에서 Compose 파일 실행

#### Step 3: 데이터베이스 마이그레이션 (Rider GUI)
1. **Tools** → `Entity Framework Core` → `Add Migration`
2. **Migration Name**: `InitialCreate`
3. **OK** → 마이그레이션 자동 생성

#### Step 4: 프로젝트 실행
1. **▶️ Run** 버튼 클릭
2. **자동으로 브라우저 열림** → Swagger UI 확인
3. **완료!** 🎉

---

## 🎮 다음 단계: Entity Framework 모델 생성

### Rider GUI로 첫 번째 엔티티 만들기

#### Step 1: Domain 엔티티 생성
1. **IdleRPG.Domain** 프로젝트 우클릭 → `Add` → `Directory`
2. **Directory Name**: `Entities`
3. **Entities 폴더** 우클릭 → `Add` → `Class`
4. **Class Name**: `Player`
5. 다음 코드 작성:
...
namespace IdleRPG.Domain.Entities
{
    public class Player
    {
        ...
        // Navigation Properties - Unity의 GetComponent와 유사
...
#### Step 2: DbContext 생성 (Rider 템플릿 활용)
1. **IdleRPG.Infrastructure** → `Add` → `Directory` → `Data`
2. **Data 폴더** 우클릭 → `Add` → `Class`
3. **Class Name**: `GameDbContext`
4. **Template**: `Entity Framework DbContext` 선택 (있다면)

이제 GUI 기반으로 깔끔하게 개발 환경이 구축되었습니다! 🚀

**Unity 개발자에게 친숙한 Rider 인터페이스**를 활용해서 콘솔 명령어 없이도 모든 설정을 완료할 수 있습니다. 다음 단계인 데이터베이스 모델링으로 넘어가실 준비가 되셨나요? 😊
...
## 🔧 Day 5-7: Entity Framework 심화 + 성능 최적화

### 🎯 학습 목표

1만 동시 접속자를 지원하기 위한 **데이터베이스 성능 최적화**와 **고급 Entity Framework 패턴**을 학습합니다.

### ⚡ 성능 최적화 핵심 패턴

#### 1. Repository Pattern + Unit of Work
```csharp
// IdleRPG.Infrastructure/Repositories/IPlayerRepository.cs
...
#### 2. 백그라운드 작업 (Hangfire)
```csharp
// IdleRPG.Infrastructure/BackgroundJobs/OfflineRewardJob.cs
using Hangfire;

namespace IdleRPG.Infrastructure.BackgroundJobs
{
    public class OfflineRewardJob
    {
        private readonly GameDbContext _context;
        private readonly ILogger<OfflineRewardJob> _logger;
        
        public OfflineRewardJob(GameDbContext context, ILogger<OfflineRewardJob> logger)
        {
            _context = context;
            _logger = logger;
        }
        ...
        /// <summary>
        /// 모든 플레이어의 오프라인 보상을 주기적으로 계산 (5분마다 실행)
        /// </summary>
        [AutomaticRetry(Attempts = 3)]
        public async Task ProcessOfflineRewards()
        {
            var cutoffTime = DateTime.UtcNow.AddMinutes(-5);
            
            // 5분 이상 오프라인인 플레이어들 조회
            var offlinePlayers = await _context.Players
                .Include(p => p.Stats)
            ...
            foreach (var player in offlinePlayers)
            {
                try
                {
                    await CalculateAndSaveOfflineReward(player);
                }
                catch (Exception ex)
                {
            ...
            _logger.LogInformation($"Processed offline rewards for {offlinePlayers.Count} players");
        }
        
        private async Task CalculateAndSaveOfflineReward(Player player)
        {
            var offlineMinutes = (DateTime.UtcNow - player.LastLogin).TotalMinutes;
            
            // 최대 12시간 보상 제한
            var rewardMinutes = Math.Min(offlineMinutes, 720); // 720분 = 12시간
            
            if (rewardMinutes < 5) return; // 5분 미만은 보상 없음
            
            var hourlyGold = player.Stats.Level * 100;
            var hourlyExp = player.Stats.Level * 50;
            
            var goldReward = (long)(rewardMinutes / 60.0 * hourlyGold);
            var expReward = (long)(rewardMinutes / 60.0 * hourlyExp);
            
            // 기존 미수령 보상과 합산
            var existingReward = await _context.OfflineRewards
                .FirstOrDefaultAsync(or => or.PlayerId == player.Id && !or.IsClaimed);
                
            if (existingReward != null)
            {
                existingReward.CalculatedGold += goldReward;
                existingReward.CalculatedExp += expReward;
                existingReward.OfflineEnd = DateTime.UtcNow;
            }
            else
            {
                _context.OfflineRewards.Add(new OfflineReward
                {
                    PlayerId = player.Id,
                    OfflineStart = player.LastLogin,
                    OfflineEnd = DateTime.UtcNow,
                    CalculatedGold = goldReward,
                    CalculatedExp = expReward,
                    BonusMultiplier = 1.0m,
                    IsClaimed = false
                });
            }
...
// 백그라운드 작업 스케줄링
app.UseHangfireDashboard("/hangfire"); // http://localhost:8080/hangfire

// 주기적 작업 등록
RecurringJob.AddOrUpdate<OfflineRewardJob>(
    "process-offline-rewards", 
    job => job.ProcessOfflineRewards(), 
    "*/5 * * * *"); // 5분마다 실행
```

#### 3. Redis 캐싱 전략
```csharp
// IdleRPG.Infrastructure/Caching/IRedisCacheService.cs
...
namespace IdleRPG.Infrastructure.Caching
{
    ...
    public class RedisCacheService : IRedisCacheService
    {
        ...
        public async Task SetAsync<T>(string key, T value, TimeSpan? expiry = null)
        {
            try
            {
                ...
                await _database.StringSetAsync(key, serializedValue, expiry ?? TimeSpan.FromHours(1));
            }
            catch (Exception ex)
            {
    ...
    {
        private readonly IPlayerService _baseService;
        private readonly IRedisCacheService _cache;
        
        public async Task<PlayerStatsDto> GetPlayerStatsAsync(Guid playerId)
        {
            var cacheKey = $"player:stats:{playerId}";
            
            // 캐시에서 조회 시도
            var cachedStats = await _cache.GetAsync<PlayerStatsDto>(cacheKey);
            if (cachedStats != null)
            {
                return cachedStats;
            }
            ...
            // 캐시 미스 시 DB에서 조회
            var stats = await _baseService.GetPlayerStatsAsync(playerId);
...
dotnet ef migrations add AddOfflineRewardSystem --startup-project ../IdleRPG.API

# 운영 환경 마이그레이션 스크립트 생성
dotnet ef migrations script --startup-project ../IdleRPG.API --output migrations.sql
```
...
# 접속 URL들
echo "🎮 게임 서버 API: http://localhost:8080/swagger"
echo "📊 Hangfire 대시보드: http://localhost:8080/hangfire"
echo "🗄️ pgAdmin: http://localhost:8082 (admin@idlerpg.com / admin123)"
```

### 성능 모니터링 설정
```csharp
// Program.cs - 프로덕션 모니터링 추가
...
3. **성능 테스트**
   - 100명의 테스트 플레이어 데이터 생성
   - 동시에 여러 API 호출 (Postman Runner 사용)
...
다음 주에는 다음 주제들을 다룰 예정입니다:

1. **JWT 인증 시스템** - 보안 강화
2. **실시간 통신** - SignalR로 멀티플레이어 기능
3. **가챠 시스템** - 확률 기반 아이템 뽑기
4. **결제 시스템** - 인앱 결제 연동
5. **부하 테스트** - 1만 동접 시뮬레이션

### 📚 추가 학습 자료

- **공식 문서**: [ASP.NET Core 가이드](https://docs.microsoft.com/aspnet/core)
- **Entity Framework**: [EF Core 성능 최적화](https://docs.microsoft.com/ef/core/performance)
- **PostgreSQL**: [PostgreSQL 튜닝 가이드](https://wiki.postgresql.org/wiki/Performance_Optimization)
- **Redis**: [Redis 캐싱 패턴](https://redis.io/docs/manual/patterns)
