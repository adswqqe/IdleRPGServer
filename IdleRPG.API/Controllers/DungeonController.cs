...
namespace IdleRPG.API.Controllers
{
    ...
    public class DungeonController : BaseController
    {
        ...
        /// <summary>
        /// 특정 캐릭터가 도전 가능한 던전 스테이지 목록을 조회합니다
        /// </summary>
        ...
        public async Task<IActionResult> ClearStage([FromBody] DungeonClearRequestDto request)
        {
            try
            {
                ...
                // 3. 던전 클리어 처리 (비즈니스 로직)
                var result = await _dungeonService.ClearStageAsync(request.CharacterId, request);
            ...
            {
                _logger.LogError(ex, "던전 클리어 처리 중 오류 - CharacterId: {CharacterId}, StageId: {StageId}",
